  ...
  return (
    <div className="space-y-6 pb-16">
      ...
      <PageHeader
        ...
        title={t('sidebar.field_reports', { defaultValue: 'Field Incident Reporting & Moderation' })}
        description={t('field_report_page.info_desc', { defaultValue: 'Ground-level hazard verification portal. Ground survey teams and local citizens can submit geolocated tensile cracks, mudslides, and boulder falls with photographic documentation.' })}
        badge={isOnline ? t('nav.online', { defaultValue: 'Online Sync Active' }) : t('nav.offline', { defaultValue: 'Offline Queueing Active' })}
        actions={
          <button
            onClick={loadReports}
            disabled={isLoading}
          ...
          >
            ...
            <span>{t('common.refresh', { defaultValue: 'Refresh Reports' })}</span>
          </button>
        }
      />
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3.5">
          <div className="p-4 rounded-xl bg-white dark:bg-[#0D0E10] border border-[#D9E2DE] dark:border-[#27272A] shadow-sm">
            <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400 block mb-1">
              {t('reports_feed.total_label', { defaultValue: 'Total Incident Reports' })}
            </span>
          ...
          <div className="p-4 rounded-xl bg-white dark:bg-[#0D0E10] border border-[#D9E2DE] dark:border-[#27272A] shadow-sm">
            <span className="text-[10px] font-bold uppercase tracking-wider text-emerald-600 dark:text-emerald-400 block mb-1">
              {t('reports_feed.status_verified', { defaultValue: 'Verified by Officials' })}
            </span>
          ...
          <div className="p-4 rounded-xl bg-white dark:bg-[#0D0E10] border border-[#D9E2DE] dark:border-[#27272A] shadow-sm">
            <span className="text-[10px] font-bold uppercase tracking-wider text-blue-600 dark:text-blue-400 block mb-1">
              {t('reports_feed.status_received', { defaultValue: 'Pending Review' })}
            </span>
          ...
          <div className="p-4 rounded-xl bg-white dark:bg-[#0D0E10] border border-[#D9E2DE] dark:border-[#27272A] shadow-sm">
            <span className="text-[10px] font-bold uppercase tracking-wider text-amber-600 dark:text-amber-400 block mb-1">
              {t('report_form.pending_sync_label', { defaultValue: 'Local Offline Queue' })}
            </span>
      ...
      {!isOfficial ? (
        <div className="max-w-3xl mx-auto">
          <SectionCard
            kicker={t('citizen.portal_sub', { defaultValue: 'Citizen Incident Portal' })}
            title={t('report_form.my_reports', { defaultValue: 'Submit & View My Reports' })}
            subtitle={t('report_form.my_reports_subtitle', { defaultValue: 'Submit new ground movement reports and check if they have been reviewed or accepted by officials.' })}
          >
        ...
        <div className="lg:col-span-6 space-y-4">
          <SectionCard
            kicker={t('report_form.reporter_type', { defaultValue: 'New Ground Observation' })}
            title={t('report_form.title', { defaultValue: 'Submit Hazard Incident' })}
            subtitle={t('report_form.subtitle', { defaultValue: 'Record geolocated ground movement, rockfall, or retaining wall deformation.' })}
          >
            <ReportForm onReportSubmitted={loadReports} />
          </SectionCard>
        </div>
        ...
        <div className="lg:col-span-6 space-y-4">
          <SectionCard
            kicker={t('reports_feed.title', { defaultValue: 'Ground Truth Verification' })}
            title={t('reports_feed.title', { defaultValue: 'Field Reports Review Feed' })}
            subtitle={t('reports_feed.subtitle', { defaultValue: 'Review incident reports submitted by ground responders and citizen observers.' })}
            badge={`${filteredReports.length} ${t('common.total', { defaultValue: 'Shown' })}`}
            actions={
              <div className="flex flex-wrap items-center gap-2">
                <div className="flex items-center gap-1">
                  ...
                  >
                    <option value="all">{t('alerts_view.filter_all', { defaultValue: 'All Statuses' })} ({reports.length})</option>
                    <option value="received">{t('reports_feed.status_received', { defaultValue: 'Pending Review' })} ({receivedCount})</option>
                    <option value="verified">{t('reports_feed.status_verified', { defaultValue: 'Verified' })} ({verifiedCount})</option>
                    <option value="dismissed">{t('reports_feed.status_dismissed', { defaultValue: 'Dismissed' })}</option>
                  </select>
                </div>
                ...
                <div className="flex items-center gap-1">
                  <select
                    ...
                    className="px-2 py-1 rounded-md bg-[#F5F7F6] dark:bg-[#141418] border border-[#D9E2DE] dark:border-[#27272A] text-[11px] font-semibold text-slate-700 dark:text-zinc-200 focus:outline-none"
                  >
                    <option value="all">{t('alerts_view.filter_all_severity', { defaultValue: 'All Severities' })}</option>
                    <option value="critical">{t('severity.critical_short', { defaultValue: 'Critical' })}</option>
                    <option value="high">{t('severity.high_short', { defaultValue: 'High' })}</option>
                    <option value="medium">{t('severity.medium_short', { defaultValue: 'Medium' })}</option>
                    <option value="low">{t('severity.low_short', { defaultValue: 'Low' })}</option>
                  </select>
                </div>
              </div>
            }
                  ...
                  >
                    ...
                    <div className="p-2.5 rounded-lg bg-white dark:bg-[#0D0E10] border border-[#D9E2DE] dark:border-[#27272A] flex flex-wrap items-center justify-between gap-2 text-[11px]">
                      ...
                      <div className="flex items-center gap-1.5">
                        <span className="font-bold text-slate-700 dark:text-zinc-300 text-[10px] uppercase font-mono flex items-center gap-1">
                          ...
                          {t('field_reports.allocate_severity', 'Allocate Severity:')}
                        </span>
                        <select
                        ...
                        >
                          <option value="low">{t('severity.low_severity', 'Low Severity')}</option>
                          <option value="medium">{t('severity.medium_severity', 'Medium Severity')}</option>
                          <option value="high">{t('severity.high_severity', 'High Severity')}</option>
                          <option value="critical">{t('severity.critical_hazard', 'Critical Hazard')}</option>
                        </select>
                      </div>
                      ...
                      <div className="flex items-center gap-1.5">
                        {r.status !== 'verified' && (
                          ...
                          >
                            ...
                            <span>{t('field_reports.verify_btn', 'Verify')}</span>
                          </button>
                        )}
                        {r.status !== 'dismissed' && (
                          <button
                          ...
                          >
                            ...
                            <span>{t('field_reports.dismiss_btn', 'Dismiss')}</span>
                          </button>
                        )}
                        <button
                        ...
                        >
                          ...
                          <span>{t('field_reports.remove_btn', 'Remove')}</span>
                        </button>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center justify-between text-[10px] text-slate-400 font-mono">
                      <div className="flex items-center gap-1">
                        ...
                        <span>{t('field_reports.gps_coords', 'GPS Coordinates:')} {r.lat?.toFixed(4)}, {r.lng?.toFixed(4)}</span>
                      </div>
                      <span>{t('field_reports.submitted_by', 'Submitted by')} {r.reporter_type || 'citizen'}</span>
                    </div>
                  </div>
                ))
              )}
